...
class SLAM(object):
    def __init__(self, intrinsics:torch.tensor, config:dict, baseline: float, checkpoint: str, init_pose: torch.tensor=torch.eye(4)):
        """
        ...
        self.pose_estimator = RAFTPoseEstimator(self.intrinsics, baseline, checkpoint, config['img_shape'], config['frame2frame'],
                                                self.init_pose)
