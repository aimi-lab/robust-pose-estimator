...
class PreProcess(object):
    ...
    def compensate_illumination(self, img, depth, intrinsics):
        ...
        normals_light_src = torch.stack((u - intrinsics[0,2],v -intrinsics[1,2], intrinsics[0,0]*torch.ones(img.shape[-2:])))
        ...
        cos_alpha = batched_dot_product(normals_light_src.reshape(3, -1).T, surface_normals.reshape(3, -1).T).reshape(1, *img.shape[-2:])
