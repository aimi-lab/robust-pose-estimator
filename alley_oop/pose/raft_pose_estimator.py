...
class RAFTPoseEstimator(torch.nn.Module):
    ...
    def estimate(self, frame: FrameClass, scene: SurfelMap):
        depth = self.resize(frame.depth)
        confidence = self.resize(frame.confidence)
        if self.frame2frame:
            ...
            rel_pose_se3 = self.model(model_frame.img, frame.img, model_depth, depth, model_conf, confidence)
            rel_pose = lie_se3_to_SE3(rel_pose_se3)
