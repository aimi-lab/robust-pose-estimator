...
class SurfelMapDeformable(SurfelMap):
    ...
    def fuse(self, *args):
        ...
        proj_ref_idx = (global_ipts[0, :] >= 0) & (global_ipts[1, :] >= 0) & \
               (global_ipts[0, :] < self.img_shape[1]-1) & (global_ipts[1, :] < self.img_shape[0]-1)

        proj_trg_idx = self.get_match_indices(global_ipts[:, proj_ref_idx])

        # fit and predict warp-field for canonical model to current frame
        proj_trg_idx, proj_ref_idx = self.estimate_warpfield(opts, proj_ref_idx, proj_trg_idx)
        # pre-select confidence elements
        conf = torch.ones_like(frame.confidence.view(1, -1)) / self.conf_thr

        # update confidences
        self.conf[:, proj_ref_idx] = torch.clamp(self.conf[:, proj_ref_idx] + conf[:, proj_trg_idx], 0.0, 1.0)  # saturate confidence to 1
    ...
    def estimate_warpfield(self, opts, ref_idx, trg_idx):
        ...
        self.warp_field[:, ref_idx] = opts[:, trg_idx] - self.opts[:, ref_idx]
        diff = torch.sum(self.warp_field ** 2, dim=0)
        not_deformed = diff <= self.d_thresh ** 2

        valid = diff[ref_idx] <= (10*self.d_thresh) ** 2
        self.warp_field[:, not_deformed] = 0.0
        ref_idx[ref_idx.clone()] &= valid
        return trg_idx[valid], ref_idx
