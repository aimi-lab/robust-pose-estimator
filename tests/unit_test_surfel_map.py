...
class SurfelMapTest(unittest.TestCase):
    ...
    def test_point_fusion(self):
        ...
        self.global_gray = self.gtruth_gray[..., 2*gap:, 2*gap:]
        self.global_dept = self.gtruth_dept[..., 2*gap:, 2*gap:]
        self.global_normals = self.gtruth_normals[2*gap+1:, 2*gap+1:]
        ...
        #self.assertTrue(surf_map.disp.numel() > self.gtruth_opts.numel(), 'Surfel map points too little')
