...
from core.geometry.pinhole_transforms import transform, project, project2image, reproject, create_img_coords_t, homogeneous
from core.pose.pose_head import DeclarativePoseHead3DNode
from core.geometry.lie_3d_small_angle import small_angle_lie_SE3_to_se3_batch, small_angle_lie_se3_to_SE3_batch
...
class PoseHeadTester(unittest.TestCase):
    ...
    def setUp(self):
        ...
        flow_off = project(self.pcl.view(n,3,-1), self.poses.inv().matrix(), intrinsics=self.kmat).view(n,2,180,180)
        ...
        self.pcl_transformed = transform(homogeneous(self.pcl.view(n,3,-1)), self.poses.inv().matrix())[:,:3].view(n,3,180,180)
