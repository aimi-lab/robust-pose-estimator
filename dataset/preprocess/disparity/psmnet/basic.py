...
class PSMNet(nn.Module):#nn.Module
    def __init__(self, params):
        super(PSMNet, self).__init__()

        self.maxdisp = int(params['maxdisp'])
...
########
    ...
    def forward(self, left, right):
        ...
        #matching
        cost = Variable(torch.FloatTensor(refimg_fea.size()[0], refimg_fea.size()[1]*2, self.maxdisp//4,  refimg_fea.size()[2],  refimg_fea.size()[3]).zero_(), volatile= not self.training).type_as(refimg_fea)
